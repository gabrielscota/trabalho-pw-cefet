	...
	class AcessoDAO extends CrudDAO
	{
		...
		public function incluir($acesso){	
			...
			try{
				...
				$run = $pdo->prepare($sql);
				$run->bindValue(':descricao', $acesso->getDescricao(), PDO::PARAM_STR); 
				$run->bindValue(':menu', $acesso->getMenu(), PDO::PARAM_STR); 
				$run->bindValue(':arquivo', $acesso->getArquivo(), PDO::PARAM_STR);  
				$run->bindValue(':tipo', $acesso->getTipo(), PDO::PARAM_STR); 	
				$run->bindValue(':idAcessoPai', $acesso->getIdAcessoPai(), PDO::PARAM_INT);				
	  			$run->execute(); 
		...
		public function listarPorAcessoPai($codigo){
			...
			try{
				...
				$run = $pdo->prepare($sql);	
				$run->bindValue(':codigo', $codigo, PDO::PARAM_INT);					
				$run->execute(); 
				$resultado = $run->fetchAll();

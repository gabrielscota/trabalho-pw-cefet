	...
	class PedidoDAO extends CrudDAO
	{
		...
		public function incluir($pedido){	
			...
			try{
				...
				$run = $pdo->prepare($sql);
				$run->bindValue(':idUsuario', $pedido->getUsuario()->getIdUsuario(), PDO::PARAM_INT);
				$run->bindValue(':dataPedido', $dataPedido, PDO::PARAM_STR); 
		...
		public function atualizar($pedido){	
			...
			try{
				...
				$run = $pdo->prepare($sql);
				$run->bindValue(':idPedido', $pedido->getIdPedido(), PDO::PARAM_INT);  
				$run->bindValue(':idUsuario', $pedido->getUsuario()->getIdUsuario(), PDO::PARAM_INT);
				$run->bindValue(':dataPedido', $dataPedido, PDO::PARAM_STR);  
	  			$run->execute(); 
